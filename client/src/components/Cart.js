...
class Cart extends Component {
  render() {
    const {products, addToCart, removeFromCart, addOrder, classes, width} = this.props;

    const displayCartForMediumScreens = (
      <Fragment>
    ...
    const displayCartForSmallScreens = (
      <Fragment>
        {products.map(({id, logo, title, price, cartCount, totalCount, usedCount}) => {
          const count = totalCount - parseInt(usedCount);
        ...
        {products.length > 0 ?
          (
            <Paper className={`${classes.paper} ${classes.product}`}>
              {width === 'xs' || width === 'sm' ? displayCartForSmallScreens : displayCartForMediumScreens}
              <Grid container>
                <Grid item xs={12}>
...
export default compose(withStyles(styles), withWidth())(Cart);