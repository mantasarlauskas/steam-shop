...
export default (WrappedComponent, fields) => {
	class WithForm extends Component {
		...
		componentDidUpdate({successMessage: prevSuccessMessage, errorMessage: prevErrorMessage}) {
			const {successMessage, errorMessage} = this.props;
		...
		handleChange = name => ({target: {value}}) => {
			this.setState(prevState => ({
				textFields: prevState.textFields.map(field =>
					field.id === name ? {...field, value, empty: value.length === 0} : field
				)
			}));
		};
		...
		renderError = () => {
			...
			return (
				<SnackbarContent
					...
					message={<span><ErrorIcon className={classes.errorIcon}/>{` ${error}`}</span>}
				/>
			);
		};
		...
		renderSuccess = () => {
			...
			return (
				<SnackbarContent
					...
					message={<span><CheckCircleIcon className={classes.errorIcon}/>{` ${success}`}</span>}
				/>
			);
		};
